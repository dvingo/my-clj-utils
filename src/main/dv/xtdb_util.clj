...
(defn put-async
  ...
  (xt/submit-tx crux-node [[::xt/put data]]))
...
(defn update-entity
  ...
  (let [ent     (xt/entity (->db crux-node) entity-id)
...
(defn q
  ...
  [crux-node query] (xt/q (->db crux-node) query))
...
(defn entity
  [crux-node entity-id]
  (xt/entity (->db crux-node) (if (fu/ref? entity-id) (second entity-id) entity-id)))
...
(defn history
  ([crux-node id sort-order]
   (xt/entity-history (->db crux-node) id sort-order
     {:with-docs? true}))
  ([crux-node id sort-order opts]
   (xt/entity-history (->db crux-node) id sort-order opts)))
...
(defn entity-at
  ...
  (xt/entity (xt/db crux-node valid-time) entity-id))
...
(defn submit-sync
  ...
  (xt/await-tx crux-node (xt/submit-tx crux-node tx)))
